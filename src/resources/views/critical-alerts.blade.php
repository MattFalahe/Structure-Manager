...
<style>
    /* === Critical Alerts — page-specific chrome ===
       Generic card / button / fuel-status / metenox-indicator come from
       canonical structure-manager.css. The bespoke severity-colored
       primitives (alert cards, dual-fuel boxes, limiting indicators,
       info-box overrides) below are functionally specific to this view
       and use SEMANTIC color tokens that MUST be preserved. */

    /* SEMANTIC alert card stripes — DO NOT CHANGE */
    .alert-card {
        border-left: 4px solid;
        margin-bottom: 1rem;
    }
    .alert-card.critical {
        border-left-color: var(--sm-danger) !important;
        background: rgba(220, 53, 69, 0.1) !important;
    }
    .alert-card.warning {
        border-left-color: var(--sm-warning) !important;
        background: rgba(255, 193, 7, 0.1) !important;
    }

    /* SEMANTIC info-box styling — DO NOT CHANGE colors. The
       AdminLTE info-box used here is forced to a high-contrast
       solid background so the danger/warning/info severity is
    ...
    /* SEMANTIC fuel-critical / fuel-warning override (this view bumps
       size + adds shadow vs the canonical small-text variant). DO NOT
       CHANGE colors. */
    .fuel-critical {
    ...
    /* Layout primitives unique to this view */
    .structure-icon {
        font-size: 2rem;
        margin-right: 1rem;
    }
    ...
    /* SEMANTIC structure-type badges (Metenox purple, POS pink) — DO NOT CHANGE */
    .badge-metenox {
    ...
    /* Limiting-factor badge — animated pulse */
    ...
    /* Large limiting-indicator chip beside the structure name */
    .limiting-indicator {
        display: inline-flex;
        align-items: center;
        gap: 0.5rem;
    ...
    /* SEMANTIC limiting-indicator color variants — DO NOT CHANGE.
       These override the canonical .limiting-fuel / .limiting-gas tokens
       intentionally because this view uses solid backgrounds for the
       large badges (vs the small chip used elsewhere). */
    .limiting-fuel {
        background: #2196f3 !important;
        border: 2px solid #0d47a1 !important;
        color: #ffffff !important;
    ...
    /* Soft-tinted variants used inside .stat-badge (limiting summary) */
        ...
        <div class="card card-dark">
            <div class="card-header">
                ...
                <div class="card-tools d-flex align-items-center" style="gap:0.5rem;">
                    ...
                    @endcan
                    <button type="button" class="btn btn-sm btn-sm-primary" id="refresh-alerts">
...
function initializeCriticalAlerts() {
    ...
    function loadAlerts() {
        ...
        $.get('{{ route("structure-manager.critical-alerts-data") }}?scope=' + smScope(), function(data) {
            let criticalCount = 0;
            let warningCount = 0;
            let totalBlocks = 0;
            ...
            data.forEach(function(alert) {
                ...
                // Urgency tiers — 3-tier model from FuelThresholds.
                // Matches list page, detail page, webhooks, and Structure Board
                // so the same structure shows the same urgency everywhere.
                const _T = window.__SM_THRESHOLDS;
                let urgencyMsg = '';
                if (hoursLeft < _T.upwell_fuel_critical_hours) {
                    urgencyMsg = '<span class="badge badge-danger"><i class="fas fa-fire"></i> Critical - Refuel immediately!</span>';
                } else if (hoursLeft < _T.upwell_fuel_warning_hours) {
                    urgencyMsg = '<span class="badge badge-warning">Warning - Schedule refuel soon</span>';
                } else {
                    urgencyMsg = '<span class="badge badge-info">Monitor</span>';
                }
                ...
                if (isPOS) {
                    ...
                    if (strontiumQty > 0 || strontiumHours > 0) {
                        let strontiumStatus = strontiumHours < window.__SM_THRESHOLDS.pos_strontium_critical ?
    ...
    // Corp scope selector (admin only)
    $('#sm-scope-filter').on('change', loadAlerts);
