...
<style>
    /* === Logistics Report — page-specific chrome ===
       Generic card / button / table / fuel-status come from canonical
       structure-manager.css. This view keeps a print stylesheet and the
       grouped system-section layout. */
    ...
    /* SEMANTIC Metenox row tint + identity badge — DO NOT CHANGE */
    .metenox-row {
    ...
    /* SEMANTIC magmatic-gas requirement text — DO NOT CHANGE */
    .gas-requirement {
        color: #c04ed4;
        font-style: italic;
        font-size: 0.85rem;
...
<div class="card card-dark">
    <div class="card-header">
        ...
        <div class="card-tools no-print">
            ...
            <select id="sm-scope-filter" class="form-control form-control-sm" style="width:auto; display:inline-block;"
                    title="Corporation scope"
                    onchange="window.location='{{ route('structure-manager.logistics-report') }}?scope='+this.value">
                <option value="mine" {{ request('scope') === 'all' ? '' : 'selected' }}>My Corporations</option>
                <option value="all" {{ request('scope') === 'all' ? 'selected' : '' }}>All Corporations</option>
            </select>
            @endcan
            <button onclick="window.print()" class="btn btn-sm btn-sm-primary">
...
$(document).ready(function() {
    ...
    // Load logistics report — forward the page's ?scope= to the data endpoint
    $.get('{{ route("structure-manager.logistics-data") }}' + window.location.search, function(data) {
        reportData = data;
        ...
        for (let system in data.systems) {
            ...
            systemData.structures.forEach(function(structure) {
                ...
                let _T = window.__SM_THRESHOLDS;
                let daysClass = hoursLeft < _T.upwell_fuel_critical_hours ? 'fuel-critical' :
                               hoursLeft < _T.upwell_fuel_warning_hours ? 'fuel-warning' : 'fuel-good';
