...
return [
    ...
    <ul>
        ...
        <li><strong>Admin scope toggle:</strong> A "My Corporations / All Corporations" dropdown in the header lets admins narrow the page to their own corps (the default) or expand to every corp on the install. Non-admins always see only their own corps and do not see the dropdown.</li>
    </ul>',
...
docker compose -f docker-compose.yml -f docker-compose.mariadb.yml -f docker-compose.traefik.yml up -d</pre>
    ...
    'id_resolver_performance' => '<strong>Performance considerations:</strong>
        <ul>
            ...
            <li>Per CCP\'s third-party developer guidelines, ESI calls include a User-Agent identifying the plugin: <code>SeAT-StructureManager/2.0.2 (+https://github.com/MattFalahe/structure-manager)</code>.</li>
        </ul>',
