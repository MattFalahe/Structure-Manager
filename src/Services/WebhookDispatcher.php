...
class WebhookDispatcher
{
    ...
    public static function resolveBindings(string $namespace, string $categoryKey, ?int $corporationId): array
    {
        ...
        // Corporation filter — ALWAYS applied as defense-in-depth.
        //
        //   webhook.corporation_id IS NULL = "all corps" (global)
        //   webhook.corporation_id = X     = scoped to corp X
        //
        // When the caller has a real corp ID, match global + that corp.
        // When the caller has no corp ID (null or 0 — typically only happens
        // for test data or a misconfigured row), match only global webhooks
        // so we never broadcast to a different corp's webhook by accident.
        //
        // The previous version had `if ($corporationId !== null) { ... }`
        // which silently became "no corp filter at all" when null was
        // passed — every callsite happens to cast to int first so it was
        // safe in practice, but a future caller passing null literally
        // would have leaked across corps. Always-applied filter eliminates
        // the latent risk.
        $query->where(function ($q) use ($corporationId) {
            $q->whereNull('w.corporation_id');
            if ($corporationId !== null && $corporationId > 0) {
                $q->orWhere('w.corporation_id', $corporationId);
            }
        });
